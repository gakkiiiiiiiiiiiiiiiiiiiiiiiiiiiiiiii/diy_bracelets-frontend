...
export function useBracelet3dMp(
	...
	function cancelMotionAnimations(object: any) {
	...
	/** 小程序端与 H5 使用同一套正视圆珠半球投影，避免方图被错误包裹到球面。 */
	...
	function updateBeads(newBeads: BraceletBead[], oldBeads: BraceletBead[] = []) {
		...
		newBeads.forEach((bead, index) => {
			...
			addOrdinal += 1;
			beadEntry.root.position.set(fromX, ADD_START_HEIGHT, fromZ);
			beadEntry.root.scale.setScalar(ADD_START_SCALE);
